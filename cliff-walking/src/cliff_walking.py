...
def sarsa(action_value_estimates, expected=False, step_size=step_size):
    ...
    while state != goal:
        ...
        action_value_estimates[state[0], state[1], action] += (step_size *
                    (reward + target - action_value_estimates[state[0], state[1], action]))
...
def q_learning(action_value_estimates, step_size=step_size):
    ...
    while state != goal:
        ...
        action_value_estimates[state[0], state[1], action] += (step_size
                                                * (reward + discount * np.max(action_value_estimates[next_state[0], next_state[1], :])
